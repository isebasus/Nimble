...
export default class ProductionProfile extends Component{
    ...
    static renderProfile(items, notFound, path, history, merch, brand, caption) {
        let item;
        if (notFound == true) {
        ...
        if (path == null) {
            ...
            if (pathArr.length != 3) {
                ...
                if (this.state.loaded == false) {
                    this.getData(pathArr[0], pathArr[1], pathArr[2]);
    ...
    async getData(company, merch, itemUrl) {
        ...
        this.setState({items: res, loading: false, merch: itemObj, brand: brandName, caption: caption});
    }
}
